<template>
  <li class="px-4 py-2 flex items-center justify-between">
    <p class="text-lg font-medium" :class="{ 'line-through': todo.complete }">
      {{ todo.title }}
    </p>
    ...
    <div class="flex gap-3 items-center">
      <label :for="todo.id">
        <input
          ...
          @input="handleInjectTodoComplete(todo.id)"
        />
        ...
        <svg
          ...
          fill="none"
          viewBox="0 0 24 24"
          class="w-5 h-5 fill-white text-white cursor-pointer"
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
        ...
        <svg
          ...
          id="todoToggle"
          viewBox="0 0 512 512"
          class="stroke-white stroke-[3rem] fill-none w-5 h-5 cursor-pointer"
        >
          <path
      ...
      <button
        ...
        @click="handleInjectTodoDelete(todo.id)"
      >
        <svg
          viewBox="0 0 24 24"
          fill="none"
          class="stroke-white stroke-[2px] fill-none w-5 h-5"
        >
...
export default {
  props: {
    todo: Object,
    handleTodoComplete: Function,
  },

  inject: {
    handleInjectTodoComplete: {
      from: "handleTodoComplete",
      default: (id) => {
        console.log(id);
      },
    },

    handleInjectTodoDelete: {
      from: "handleDeleteTodo",
      default: (id) => {
        console.log(id);
      },
    },
